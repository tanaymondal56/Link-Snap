...
const UserDetailsModal = ({ isOpen, onClose, user }) => {
  ...
  return createPortal(
    ...
    >
      ...
      <div
        data-modal-content
        className="relative w-full max-w-2xl bg-gray-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90dvh] overscroll-contain animate-scale-in"
      >
        {/* Header */}
                  ...
                  <div className="p-4 bg-gray-800/50 rounded-xl border border-white/5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-black/20 p-3 rounded-lg">
                        ...
                        <p
                          className={`font-medium ${user.isVerified ? 'text-green-400' : 'text-yellow-400'} flex items-center gap-1.5`}
                        >
                      ...
                      <div className="bg-black/20 p-3 rounded-lg">
                        ...
                        <p
                          className={`font-medium ${user.isBanned ? 'text-red-400' : 'text-green-400'} flex items-center gap-1.5`}
                        >
                ...
                <div className="space-y-4">
                  {banHistory.length === 0 ? (
                    ...
                    banHistory.map((event) => (
                      <div
                        key={event._id}
                        className="bg-gray-800/50 rounded-xl p-4 border border-white/5 relative overflow-hidden group"
                      >
                        <div
                          className={`absolute left-0 top-0 bottom-0 w-1 ${event.type === 'ban' ? 'bg-red-500' : 'bg-green-500'}`}
                        />
                  ...
                  <div className="bg-violet-500/10 border border-violet-500/20 p-4 rounded-xl flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-white">
                        @{usernameHistory.currentUsername || 'None'}
                      </p>
                      {usernameHistory.usernameChangedAt && (
                        <p className="text-xs text-gray-500 mt-0.5">
                          Last changed: {formatDateTime(usernameHistory.usernameChangedAt)}
                        </p>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="space-y-3">
                      {usernameHistory.history?.length === 0 ? (
                        <p className="text-sm text-gray-500 italic">
                          No previous usernames recorded.
                        </p>
                      ) : (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <span className="text-gray-500 font-mono text-xs">
                                #{usernameHistory.history.length - i}
                              </span>
