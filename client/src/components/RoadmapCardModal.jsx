...
const RoadmapCardModal = ({ item, config, onClose, onUpvote, isVoting }) => {
  ...
  return createPortal(
    <div 
      className="fixed inset-0 z-[1000] overflow-y-auto bg-black/75 backdrop-blur-md animate-fade-in"
    >
      <div 
      ...
      >
        ...
        <div 
          data-modal-content
          className="relative w-full max-w-2xl bg-gray-900 border border-white/10 rounded-2xl sm:rounded-3xl shadow-2xl flex flex-col max-h-[85dvh] sm:max-h-[90vh] overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-200 overscroll-contain"
        ...
        >
          ...
          <button
            ...
            className="absolute top-3 right-3 sm:top-4 sm:right-4 z-50 p-2 sm:p-2.5 rounded-full bg-gray-800/80 text-gray-300 hover:text-white border border-white/10 backdrop-blur-md shadow-lg transition-all active:scale-95"
          >
            <X className="w-5 h-5" />
          </button>

          <div className="p-5 pb-4 sm:p-8 sm:pb-6 overflow-y-auto custom-scrollbar relative z-10">
            
            {/* Status & Version Header */}
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-5 sm:mb-6 pr-10">
            ...
            {/* Progress Bar */}
            <div className="mb-6 sm:mb-8 bg-white/5 border border-white/5 rounded-2xl p-4">
            ...
            {/* Upvote & Action Bar */}
            <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-3 mb-6 sm:mb-8 p-4 bg-white/5 border border-white/5 rounded-2xl">
              <button
            ...
            {item.changes && item.changes.length > 0 && (
              <div>
                <h3 className="text-base sm:text-lg font-semibold text-white mb-3.5 flex items-center gap-2">
                  <Tag className="w-5 h-5 text-purple-400" />
                  Planned Updates & Scope
                </h3>
