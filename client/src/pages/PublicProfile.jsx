...
const THEMES = {
  ...
  sunset: {
    ...
    button:
      'bg-gradient-to-r from-orange-500 via-rose-500 to-pink-500 hover:from-orange-400 hover:via-rose-400 hover:to-pink-400',
    buttonText: 'text-white',
    text: 'text-white',
  ...
  switch (platform) {
    case 'twitter':
      return cleanValue.startsWith('http')
        ? cleanValue
        : `https://twitter.com/${cleanValue.replace(/^@/, '')}`;
    case 'instagram':
      return cleanValue.startsWith('http')
        ? cleanValue
    ...
    case 'github':
      return cleanValue.startsWith('http')
        ? cleanValue
        : `https://github.com/${cleanValue.replace(/^@/, '')}`;
    case 'tiktok':
      if (cleanValue.startsWith('http')) return cleanValue;
...
const Badge = ({ children, tooltip, gradient, shadowColor }) => {
  ...
  return (
    ...
    >
      <span
        className={`px-2.5 py-0.5 text-xs font-bold bg-gradient-to-r ${gradient} text-white rounded-full shadow-lg ${shadowColor} transition-transform active:scale-95 select-none`}
      >
        {children}
      </span>
  ...
  const customButtonStyles =
    profile?.theme === 'custom' && profile?.customTheme
      ? {
          ...
          borderRadius:
            profile.customTheme.buttonStyle === 'pill'
              ? '9999px'
              : profile.customTheme.buttonStyle === 'square'
                ? '12px'
                : '20px',
        }
      : {};
    ...
    <LazyPullToRefresh onRefresh={() => window.location.reload()}>
      ...
      >
        ...
        <Helmet>
          ...
          <meta
            name="description"
            content={profile?.bio || `Check out ${displayName}'s links on Link Snap`}
          />
          <meta property="og:title" content={`${displayName} | Link Snap`} />
          <meta
            property="og:description"
            content={profile?.bio || `Check out ${displayName}'s links`}
          />
        ...
        <div className="relative z-10 max-w-lg mx-auto px-4 py-8 min-h-screen flex flex-col">
          ...
          <div className={`${theme.card} border rounded-[2rem] p-8 mb-8 relative overflow-hidden`}>
            ...
            <div
              className={`absolute top-0 left-1/2 -translate-x-1/2 w-24 h-1 bg-gradient-to-r ${theme.accent} rounded-full`}
            />
            ...
            <div className="flex flex-col items-center mb-6">
              <div className="relative mb-4">
                {/* Glow effect */}
                <div
                  className={`absolute inset-0 bg-gradient-to-r ${theme.accent} rounded-full blur-xl opacity-50 scale-110`}
                />
                {profile?.avatar && !avatarError ? (
                  ...
                  <div
                    className={`relative w-28 h-28 rounded-full bg-gradient-to-br ${theme.accent} flex items-center justify-center text-4xl font-bold text-white ring-4 ring-white/10`}
                  >
          ...
          <div className="flex-1 space-y-3">
            {profile?.links?.map((link, index) => (
              ...
              >
                ...
                >
                  <span className="block truncate font-semibold">{getMaskedTitle(link.title)}</span>
                  {link.shortCode && (
        ...
        {showQR && (
          ...
          >
            ...
            >
              ...
              <h3 className={`text-xl font-bold ${theme.text} mb-6 text-center`}>Scan to visit</h3>
