          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/5">
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Check links for malware/phishing (Google API)
                </p>
              </div>
              <button
                onClick={async () => {
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        const { data } = await api.patch('/admin/settings', {
                          safeBrowsingAutoCheck: newValue,
                        });
                        setSettings(data);
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const { data } = await api.post('/admin/settings/scan', {
                          type: 'unchecked',
                        });
                        showToast.dismiss(toastId);
                        showToast.success(
                          `Scan complete: Processed ${data.details?.processed || 0} links`
                        );
                      } catch (err) {
                        showToast.dismiss(toastId);
                        console.error(err);
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        showToast.dismiss(toastId);
                        showToast.success(
                          `Retry complete: Processed ${data.details?.processed || 0} links`
                        );
                      } catch (err) {
                        showToast.dismiss(toastId);
                        console.error(err);
