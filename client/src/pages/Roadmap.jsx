...
const Roadmap = () => {
  ...
  return (
    <PullToRefresh onRefresh={() => fetchRoadmap(1)}>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            See what we're working on and what's planned for Link Snap. Features move from ideas to reality.
          </p>
        </div>

        {/* Search & Filter Bar */}
        {!loading && !error && roadmapData && (roadmapData.total > 0 || roadmapData.items.length > 0) && (
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-center mb-10">
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              ...
              {Object.entries(statusConfig).map(([key, config]) => {
                ...
                const count = roadmapData.counts ? roadmapData.counts[key] : (roadmapData.grouped[key]?.length || 0);
        ...
        {/* Empty State */}
        {!loading && !error && (!roadmapData || (roadmapData.items.length === 0 && roadmapData.pagination?.totalItems === 0)) && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-400 text-center max-w-md">
              No upcoming features on the roadmap right now. Check back soon or view our changelog to see what we've already shipped!
            </p>
            <Link
              to="/changelog"
        ...
        {/* No Search Results */}
        {!loading && !error && roadmapData && roadmapData.items.length > 0 && filteredItems.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            {Object.entries(groupedItems).map(([status, items]) => {
              ...
              return (
                <div key={status} className="flex flex-col">
                  {/* Column Header */}
                  <div className={`flex items-center gap-2 mb-4 p-3 rounded-xl ${config.bgColor} border ${config.borderColor}`}>
                    ...
                    <span className={`ml-auto px-2 py-0.5 rounded-full text-xs font-bold ${config.bgColor} ${config.textColor}`}>
                      {items.length}
                    </span>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-start gap-3 mb-4">
                            <div className={`p-2.5 rounded-xl bg-gradient-to-br ${config.color} shrink-0 shadow-lg`}>
                              <IconComponent className="w-5 h-5 text-white" />
                            </div>
                            <div className="flex-1 min-w-0 pt-0.5">
                              <div className="flex items-center gap-2 flex-wrap mb-1.5">
                                <span className={`px-2.5 py-0.5 rounded-full text-[10px] sm:text-xs font-bold bg-gradient-to-r ${config.color} text-white shadow-sm`}>
                                  v{item.version}
                                </span>
                              ...
                              <h4 className="font-bold text-white text-base leading-snug group-hover:text-purple-200 transition-colors">{item.title}</h4>
                            </div>
                          </div>
                          
                          {/* Description */}
                          {item.description && (
                            <p className="text-gray-400/90 text-sm mb-4 leading-relaxed line-clamp-3 ml-1">{item.description}</p>
                          )}
                            ...
                            {item.changes?.slice(0, 4).map((change, idx) => {
                              ...
                              return (
                                <div key={idx} className="flex items-start gap-2.5 text-xs">
                                  <span className="shrink-0 mt-0.5 text-base">{changeStyle.emoji}</span>
                                  <span className="text-gray-300 leading-relaxed font-medium">{change.text}</span>
                                </div>
                              );
                            })}
        ...
        {/* Load More Button */}
        {!loading && !error && roadmapData?.pagination?.hasMore && selectedStatus === 'all' && !searchQuery && (
              ...
              {loadingMore ? (
                ...
                <>
                  ...
                  <div className="px-2 py-0.5 rounded-full bg-white/10 text-xs">
                    {(roadmapData.pagination.totalItems || 0) - (roadmapData.items?.length || 0)} remaining
                  </div>
                </>
              )}
            </button>
